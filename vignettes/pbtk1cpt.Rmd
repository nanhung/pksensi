...
```{r, eval=T, fig.cap = '**Figure 1.** Simulation results of one-compartment PK model.'}
plot(y)
```
...
```{r, eval=T, fig.cap = '**Figure 2.** The parameter search curve generated by eFAST method. The x-axis is the sample number and y-axis is the parameter value for `vdist`, \`ke`, `kgutabs`, and \`BW` (top to down), respectively. The bar plot shows the distribution of sampling parameter.', fig.height=5}
...
```{r, eval=T, fig.cap = '**Figure 3.** Time-dependent SI of the plasma concentration estimated from one-compartment PK model during 24-hr time period intake. The solid line represent the total (black) and first (red) order SI with 95\\% confidence interval (polygon). The dash line is the cut-off with default value of 0.05.'}
plot(out)
```
...
```{r, eval=T, fig.cap = '**Figure 4.** Time-dependent SI of the plasma concentration estimated from one-compartment PK model during 24-hr time period intake.'}
...
```{r, eval=T, fig.cap = '**Figure 5.** The relationship between model parameter and estimated concentration at times 0.01, 2.01, 6.01, and 24.01 hr (top to bottom), respectively.'}
